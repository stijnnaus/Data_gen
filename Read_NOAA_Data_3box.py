...
This version is modified. The atmosphere is now represented by 3 boxes:
a tropical and 2 extratropicals. The result will be used for my 3-box experiment.
...
boxes = [ [-90., -23.], [-23., 23.], [23., 90]]
nbox = len(boxes)
styear, edyear = 1992, 2016
mcf_yrs = range(styear, edyear)
ar_ex,ar_tr = np.sin(90)-np.sin(23), 2*np.sin(23)
w_area = [ar_ex, ar_tr, ar_ex] # Weights per box based on area per box
w_trop = [ar_ex, ar_tr*1.5, ar_ex] # Weights per box based on area per box times tropopause height
...
box_means, glob_av_mon = glob_av( stat_mon, yavs_mon, stat_chars, w_area, sty=styear, edy=edyear, bm=True )
box_mins = np.swapaxes(box_means,0,1)
box_rats = box_mins/box_mins[2]
box_rats = box_mins/(np.sum(box_mins,axis=0))
...
ax1.set_title('MCF concentration in each box')
ax2 = fig.add_subplot(212)
ax2.set_title('Ratio between boxes relative to box 2 (NH extratopics)')
box_0 = box_means[:,2]
for ib in range(nbox):
    box_m = box_means[:,ib]
    ax1.plot(mcf_noaa_yrs, box_m, 'o-' , label=boxnames[ib])
    ax2.plot(mcf_noaa_yrs, box_m/box_0, 'o-', label=boxnames[ib])
ax1.legend(loc='best')
ax2.legend(loc='best')
plt.savefig('3box_MCF.png')
